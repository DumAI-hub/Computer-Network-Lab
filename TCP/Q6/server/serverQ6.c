...
int main() {
  ...
  while (1) {
    ...
    if ((c1_sock = accept(sfd, (struct sockaddr *)&s, (socklen_t *)&slen)) <
        0) {
    ...
    if ((c2_sock = accept(sfd, (struct sockaddr *)&s, (socklen_t *)&slen)) <
        0) {
    ...
    for (int i = 0; buff[i] != '\0'; i++) {
      ...
      // isalpha is provided by ctype.h, it checks whether the input is a
      // alphabet or not so if isalpha is true it will look for vowels and
      // consonants
      if (isalpha(buff[i])) {
        if (isVowel(buff[i])) {
          vowels[v++] = buff[i];
